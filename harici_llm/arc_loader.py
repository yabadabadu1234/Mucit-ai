...
class GenelSohbetBicimlendirici:
    """
    Model-ailesine ozgu (RWKV-7 G1, Mamba-Codestral, Falcon-Mamba, ...) sohbet
    sablonunu ve arac-cagirma sistem promptunu tokenizer'in kendi
    apply_chat_template'i uzerinden uygular. Orijinal notebook'taki
    QwenFormatter'in yerini alir: Qwen'e sabitlenmis <|im_start|>/<|im_end|>
    token'lari yerine, model_yapilandirmalari.py'deki her aile icin ayri
    tanimlanmis sablonlari kullanir; boylece RWKV/Mamba/Falcon-Mamba ile de
    dogru sekilde calisir.
    """

    def __init__(self, tokenizer, model_ailesi: str):
        self.tokenizer = tokenizer
        self.model_ailesi = model_ailesi

        from model_yapilandirmalari import kullanici_donusu_sar, asistan_donusu_sar, cevap_sonu_isaretleri

        self._kullanici_sar = kullanici_donusu_sar
        self._asistan_sar = asistan_donusu_sar
        self._cevap_sonu = cevap_sonu_isaretleri(model_ailesi)

    def fmt_query(self, query) -> str:
        grid_input = convert_grid_to_string(query[0]["input"])
        return self._kullanici_sar(grid_input, self.model_ailesi)

    def fmt_reply(self, reply) -> str:
        return convert_grid_to_string(reply[0]) + self._cevap_sonu
    ...
    def fmt_train(self, train, last_is_challenge=False) -> str:
        ...
        for x in train:
            ...
            text += self._kullanici_sar(grid_input, self.model_ailesi)
            text += self._asistan_sar(grid_output, self.model_ailesi) + self._cevap_sonu
        if test is not None:
