...
from .denetleyici import (Baglam, DenetimHatasi, denetle_t as denetle,
                          sentezle)
...
def dogrula_hepsi() -> List[Dict[str, str]]:
    ...
    neticeler.append(_dene("uaβ: transport (ua sucEquiv) 3 = 4",
                           lambda: _esit_dene(
                               L.tasi(L.ua(S.Tamsayi(), S.Tamsayi(),
                                           L.ardil_denkligi()),
                                      S.tam_sayi(3)), S.tam_sayi(4))))
    for n in (2, 3, -1):
        neticeler.append(_dene(
            "π₁(S¹): sarim(dongu^%d) = %d" % (n, n),
            lambda n=n: _esit_dene(L.sarim(L.dongu_kuvveti(n)),
                                   S.tam_sayi(n))))
    return neticeler
...
def _esit_dene(a: Terim, b: Terim) -> None:
    if not C.esdeger_mi(a, b):
        raise AssertionError("%s ≠ %s" % (C.nf(a), C.nf(b)))


def _ua_tasima_dene() -> None:
    """NbE'de ua boyunca taşıma HESAPLANIR; somut sınama dogrula_hepsi'de."""
    A, B, e, x = D("A"), D("B"), D("e"), D("x")
    g = Baglam.terimlerden({"A": U, "B": U, "e": L.denklik_tipi(A, B), "x": A})
    C.geri_oku(g.d(L.tasi(L.ua(A, B, e), x)))
