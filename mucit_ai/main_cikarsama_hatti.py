...
Ana Çıkarım/Test Yürütücü Modülü (main_cikarsama_hatti.py)
================================================================================
Bu modül; eğitilmiş model ağırlıklarını yükleyen, test girdilerini (metin veya
ARC-AGI JSON ızgaraları) topolojik veri kontratlarına dönüştüren, R-adımlı gizil
uzay akıl yürütmesi sonrasında Chebyshev spektral projeksiyonu ile tüm cevabı
milisaniyeler mertebesinde simültane üreten çıkarım hattıdır.
"""
...
def kur_logging_sistemi(log_dosyasi: str = 'cikarsama_hatti_icra.log') -> None:
    ...
    # 1. Döner Dosya Handler'ı (Maksimum 10 MB)
...
# ==============================================================================
# I. HARİCİ DÖNÜŞTÜRÜCÜLER VE ÇIKARSAMA YÖNETİCİLERİ
# ==============================================================================
class Arc_IzgaraDonusturucu:
    """ARC-AGI Izgara Formatını Metin Akışına Dönüştürücü"""
...
class Arc_CiktiIzgaraInsaEdici:
    ...
    def insa_et(self, olasilik_matrisi: E12_ParalelTokenOlasilikMatrisi, hedef_boyut: Tuple[int, int] = (3, 3)) -> Dict[str, List[List[int]]]:
        ...
        for r in range(rows):
            row = []
            for c in range(cols):
                val = int(preds[idx % len(preds)]) % 10  # ARC renk kodları (0-9)
                row.append(val)
                idx += 1
            izgara.append(row)
...
class Model_Agirlik_ve_CikarsamaYoneticisi:
    """Model Ağırlıkları Yükleme ve Eval Yönetimi (NPZCheckpointManager Entegreli)"""
    ...
    def yukle(self, model_dizini: str, model: BiliselKanvasModeli) -> Tuple[BiliselKanvasModeli, bool]:
        ...
        # Öncelikli Kaggle notebook çıktı dizinleri
        priority_paths = [
            model_dizini,
...
from paralel_gpu import GPU_Tespitci, GPU_CikarsamaDagitimcisi

# ==============================================================================
# II. ANA ÇIKARSAMA YÜRÜTÜCÜ SÜRECİ (MAIN EXECUTION)
# ==============================================================================
def Main_CikarsamaYurutucu(model_yolu: str = "./checkpoints", test_girdisi: str = "Ahmet eve gitti.") -> str:
    baslangic_zamani = time.time()
    zaman_siniri_saniye = 900.0  # Zaman sınırı 900 saniyeye güncellendi (Kaggle/Üretim Marjı)
    
    logger.info("================================================================================")
    logger.info("BİLİŞSEL KANVAS TOPOLOJİK REKÜRENS MİMARİSİ ÇIKARSAMA HATTI BAŞLATILIYOR")
    logger.info("================================================================================")

    # 0. GPU Tespiti ve Çıkarım Dağıtıcı Kurulumu
    gpu_tespitci = GPU_Tespitci()
    gpu_cikarsama = GPU_CikarsamaDagitimcisi(gpu_tespitci)

    # 1. Konfigürasyon ve Cihaz Ayarı
    config = Model_TopolojikKonfigurasyon()
    config.device = str(gpu_tespitci.device)
    config.batch_size = 1  # Çıkarım tekil örnek üzerinden yapılır
    ...
    model = gpu_cikarsama.cikarsama_modeli_hazirla(model)

    # Restore sonrası Stiefel dikgenliğini tazele
    stiefel_izdusurucu = N13_StiefelManifolduIzdusumu()
    
    # DataParallel sarmalaması varsa kök modele eriş
    eval_model = model.module if isinstance(model, nn.DataParallel) else model
    stiefel_izdusurucu.izdüsür(eval_model.n3_lif.phi_matrisleri)

    # 3. Girdi Tespiti ve Topolojik Dönüştürme
    is_arc_json = False
    ...
    # 4. Çıkarım İleri Besleme Akışı (12 Adımlı Şemaya Birebir Uyumlu İcra)
    with torch.no_grad():
        ...
        # [N2] TopoX Hücre Kompleksi -> E3 (D1, D2) (Keyfiyet Projeksiyonu: mode='eval')
        ...
        # [N3] Sınırlama Matrisi Atama -> E4 (Phi Dict, x^(0))
        ...
        for r in range(1, config.R + 1):
            ...
            # [N4] Sorgu Seçici (Q_theta: 4-Şartlı Rasyonel Merak Vektör Sahası) -> E6
            ...
            # [N5] Cevap Süzücü (A_phi: VSA Unbinding Bellek Okuyucu) -> E7
            ...
            # [N6] Kohomolojik Aktüatör (H^1-Combat Abdisyon Hücresi & Sheaf-KAN Süperpozisyonu) -> E8
            ...
            # Döngü Başına Geri Besleme (Feedback Update)
        ...
        # [N8] Chebyshev Katsayı Projeksiyon Kafası -> E10 (C: Külli Mana Matrisi)
        ...
        # [N8_B] Dinamik Uzunluk Seçici (Arc-Length & Parametrik Kafa Hibrit) -> E10_B (N*)
        ...
        # [N9] Dinamik Chebyshev CGL Vandermonde Çarpım İstasyonu -> E11 (Gömülü Yörünge)
        ...
        # [N10] Sözlük Softmax İzdüşüm Kafası -> E12 (Token Olasılıkları)
        ...
        # 5. Çıktı İnşası ve Akıllı UTF-8 Byte Deşifreleme
        if is_arc_json:
