...
class Maarif_NedenselSuzgec(nn.Module):
    """
    [Maarif Nedensel Süzgeç] Chebyshev Spektral Non-Autoregressive Kanvas Mimarisi için
    Sürekli Volterra Tipi Nedensel Kaydırma Operatörü (Continuous Volterra Causal Shift Kernel).
    
    j > i kesikli -inf maskeleri yerine Chebyshev-Gauss-Lobatto (CGL) düğüm noktalarında (t_k = -cos(k*pi/(N-1)))
    kesintisiz Volterra nedensellik çekirdeği K(t_k, t_j) = w_j * exp(-gamma * (t_k - t_j)) uygulanır.
    Böylece Chebyshev spektral dikgenliği ve külli mana bütünlüğü %100 muhafaza edilir.
    """
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        super().__init__()
        self.config = config
    ...
    def forward(self, X_output: torch.Tensor) -> torch.Tensor:
        ...
        # CGL Düğüm Noktaları t_k in [-1, 1]
        k_indices = torch.arange(N, dtype=torch.float32, device=device)
        t = -torch.cos(k_indices * math.pi / max(1, N - 1))  # [N]

        # Continuous Volterra Causal Matrix: K_volterra[k, j] = w_j * exp(-gamma * (t_k - t_j)) for j <= k, else 0
        t_diff = t.unsqueeze(1) - t.unsqueeze(0)  # t_k - t_j [N, N]
        w_j = math.pi / max(1, N - 1)
        gamma_clamped = F.softplus(self.gamma) + 1e-4

        # Mask for j <= k (causal past/present, continuous decay)
        causal_trig_mask = (t_diff >= 0).float()
        K_volterra = w_j * torch.exp(-gamma_clamped * t_diff) * causal_trig_mask  # [N, N]
        K_volterra = K_volterra / (K_volterra.sum(dim=-1, keepdim=True) + 1e-8)  # Satır bazlı L1 normalizasyonu

        # X_volterra[b, k, d] = sum_j K_volterra[k, j] * X_t[b, j, d]
        X_volterra = torch.matmul(K_volterra.unsqueeze(0), X_t)  # [B, N, d]
...
class Bellek_BaglamYoneticisi(nn.Module):
    ...
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        super().__init__()
        self.config = config
        self.M_state = nn.Parameter(torch.randn((config.batch_size, config.d_m, config.K), device=config.device))
        self.gate_net = nn.Sequential(
...
class N1_HibritByteTokenAyristirici(nn.Module):
    ...
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        ...
        for m in range(1, 33):
            W_init = torch.randn((config.d_v, config.d_v), device=config.device)
            self.Q_dict[str(m)] = nn.Parameter(stiefel_qr_projection(W_init))
    ...
    def forward(self, girdi: E1_HamMetinAkisi) -> Tuple[E2_ByteTensoru, torch.Tensor]:
        ...
        for i, tok_id in enumerate(token_ids):
            ...
            if key_m not in self.Q_dict:
                # Dinamik olarak yeni uzunluk için Stiefel Matrisi üret
                W_dyn = torch.randn((self.config.d_v, self.config.d_v), device=self.config.device)
                self.Q_dict[key_m] = nn.Parameter(stiefel_qr_projection(W_dyn))
...
class N2_TopoXHucreOlusumu(nn.Module):
    ...
    def forward(self, girdi: E2_ByteTensoru, x_initial: Optional[torch.Tensor] = None, mode: str = 'train', D0_base: Optional[torch.Tensor] = None) -> E3_SinirOperatorleri:
        ...
        # 5. HAKİKİ CEBİRSEL MÜHÜRLENME (SVD Truncated Pseudoinverse ile P_ker)
        if F_num > 0 and E > 0:
            I_E = torch.eye(E, dtype=torch.float32, device=device)
            D1T_D1_raw = torch.matmul(D1.T, D1)
            D1_pinv = torch.linalg.pinv(D1T_D1_raw, rcond=1e-5)
            P_ker = I_E - torch.matmul(D1, torch.matmul(D1_pinv, D1.T))
            D2 = torch.matmul(D2, P_ker)

        # 6. HOMOTOPİ AKSİYOMU SIFIR KONTROLÜ (ASSERT)
        if D2.shape[0] > 0 and type(D1).__name__ != 'MockTensor':
            homotopy_check = torch.matmul(D2, D1)
            err = torch.max(torch.abs(homotopy_check)).item() if homotopy_check.numel() > 0 else 0.0
            assert err < 1e-4, f"[Cebirsel Topoloji İhlali] D2 * D1 = {err} != 0.0 Homoloji aksiyomu sağlanamadı!"
...
class N3_LifSinirlamaAtama(nn.Module):
    ...
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        super().__init__()
        self.config = config
        self.phi_base = nn.Parameter(torch.randn((config.d_e, config.d_v), device=config.device))
        ...
        with torch.no_grad():
            self.phi_base.copy_(stiefel_qr_projection(self.phi_base.data))

    @property
    def phi_matrisleri(self) -> torch.Tensor:
...
class N6_KohomolojikAktor(nn.Module):
    ...
    def hesapla_moore_penrose_psodoters_vektor_etkisi(self, c_defect: torch.Tensor, D0: torch.Tensor, P: int = 5) -> torch.Tensor:
        r"""
        [CHEBYSHEV SPEKTRAL MOORE-PENROSE SOLVER]
        Warm-Start Persistent Power Iteration ile lambda_max hesaplayıp spektrumu [-1, 1] aralığına ölçekler
        ve Chebyshev polinom serisi ile (D_0 D_0^T + eps I)^(-1) operatörünü hesaplar.
        """
        device = D0.device
        B, E_dim = c_defect.shape

        eps_adaptive = max(1e-4, 1e-3 * float(c_defect.norm().detach().item()))
        M = torch.matmul(D0, D0.T) + eps_adaptive * torch.eye(E_dim, device=device, dtype=D0.dtype)

        # Warm-Start Persistent Power Iteration ile lambda_max kestirimi
        if self.v_pow_persistent is None or self.v_pow_persistent.shape[0] != E_dim or self.v_pow_persistent.device != device:
            v_pow = torch.randn((E_dim, 1), device=device, dtype=D0.dtype)
        else:
            v_pow = self.v_pow_persistent.to(device=device, dtype=D0.dtype)

        for _ in range(2):
            v_pow = torch.matmul(M, v_pow)
            v_pow = v_pow / (v_pow.norm() + 1e-8)

        self.v_pow_persistent = v_pow.detach()
        lambda_max = float(torch.matmul(v_pow.T, torch.matmul(M, v_pow)).item()) + 1e-4

        # Neumann / Chebyshev Convergent Inverse Series (I - M_norm)^k
        alpha_scale = 1.0 / (lambda_max + 1e-4)
        M_norm = alpha_scale * M
        I_e = torch.eye(E_dim, device=device, dtype=D0.dtype)
        M_diff = I_e - M_norm
        ...
        for _ in range(1, P + 1):
            S_inv = S_inv + M_pow
            M_pow = torch.matmul(M_pow, M_diff)

        u = alpha_scale * torch.matmul(c_defect, S_inv)
    ...
    def forward(self, mevcut_durum: E5_A_MevcutGizilDurum, sorgu: E6_GizilSorgu, lokal_bilgi: E7_LokalBilgi, 
        ...
        # 2. Ön-kayıtlı Projeksiyon Katmanları İle Birebir Hizalama
        if K_adjoint.shape[-1] == d_v:
            K_adj_field = K_adjoint
        elif self.adj_proj_layer.in_features == K_adjoint.shape[-1]:
            K_adj_field = self.adj_proj_layer(K_adjoint)
        else:
            K_adj_field = K_adjoint[:, :d_v] if K_adjoint.shape[-1] >= d_v else F.pad(K_adjoint, (0, d_v - K_adjoint.shape[-1]))

        if laplacian_grad.shape[-1] == d_v:
            lap_grad_field = laplacian_grad
        elif self.lap_proj_layer.in_features == laplacian_grad.shape[-1]:
            lap_grad_field = self.lap_proj_layer(laplacian_grad)
        else:
            lap_grad_field = laplacian_grad[:, :d_v] if laplacian_grad.shape[-1] >= d_v else F.pad(laplacian_grad, (0, d_v - laplacian_grad.shape[-1]))
...
class N10_SozlukSoftmaxIzdusem(nn.Module):
    """[N10] Sözlük Softmax İzdüşüm Kafası ve Nedensel Süzgeç (Dilimlenmiş Sıralı Softmax ile OOM Korumalı)"""
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        super().__init__()
        self.config = config
        self.nedensel_suzgec = Maarif_NedenselSuzgec(config)
        self.vocab_head = nn.Linear(config.d, config.V_size)

    def forward(self, gomulu_vektorler: E11_ParalelGomuluVektorlerMatrisi) -> E12_ParalelTokenOlasilikMatrisi:
        X_refined = self.nedensel_suzgec(gomulu_vektorler.X_output)
        ...
        if N > chunk_size:
            ...
            for i in range(0, N, chunk_size):
                X_chunk = X_t[:, i:i+chunk_size, :]
                logits_chunk = self.vocab_head(X_chunk)
                logits_norm = (logits_chunk - logits_chunk.mean(dim=-1, keepdim=True)) / (logits_chunk.std(dim=-1, keepdim=True) + 1e-6)
                P_chunk = torch.softmax(torch.clamp(logits_norm, min=-50.0, max=50.0), dim=-1).transpose(1, 2)
                P_chunks.append(P_chunk)
            P = torch.cat(P_chunks, dim=2)
        else:
            logits = self.vocab_head(X_t)
            logits_norm = (logits - logits.mean(dim=-1, keepdim=True)) / (logits.std(dim=-1, keepdim=True) + 1e-6)
            P_vocab = torch.softmax(torch.clamp(logits_norm, min=-50.0, max=50.0), dim=-1)
            P = P_vocab.transpose(1, 2)
        return E12_ParalelTokenOlasilikMatrisi(P=P)
...
class Riyazi_StiefelManifolduIzdusumu:
    ...
    def izdusur(self, phi_dict: Union[Dict[str, torch.Tensor], nn.ParameterDict, torch.Tensor, Any]):
        with torch.no_grad():
            ...
            elif isinstance(phi_dict, torch.Tensor) and phi_dict.dim() == 2:
                phi_dict.copy_(stiefel_qr_projection(phi_dict))
...
class N14_OdulTopolojikDevresmezlikMotoru:
    """[N14] GRPO Topolojik Tutarlılık ve Ödül Skorlama Motoru"""
    def __init__(self, config: Model_TopolojikKonfigurasyon):
        self.config = config
    ...
    def hesapla(self, P: torch.Tensor, hedefler: torch.Tensor) -> torch.Tensor:
        ...
        return is_correct * 2.0 + topolojik_invaryant * 0.5
    ...
    def hesapla_aktif_sorgu_odulu(self,
        ...
        if not hasattr(self, 'W_up_stiefel') or self.W_up_stiefel is None or self.W_up_stiefel.device != x_context.device or self.W_up_stiefel.shape != (d_a, d_v):
            W_raw = torch.randn(d_a, d_v, device=x_context.device, dtype=x_context.dtype)
            Q, _ = torch.linalg.qr(W_raw)
            self.W_up_stiefel = Q  # Stiefel dikgen matrisi St(d_a, d_v)
        
        # Her bir v düğümünün a_r cevabıyla mesafe vektörü [B, V]
        x_nodes_proj = torch.matmul(x_nodes_all, self.W_up_stiefel.T)  # [B, V, d_a]
...
class Kayip_VICReg_UcluBilgiKorunumu(nn.Module):
    ...
    def rekonstruksiyon_kaybi_vektor(self, x: torch.Tensor, z: torch.Tensor, W: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if W is not None:
            ...
            return torch.mean((x - x_rec) ** 2, dim=0)  # [D]
        else:
            if x.shape == z_flat.shape:
                return torch.mean((x - z_flat) ** 2, dim=0)  # [D]
            elif x.shape[-1] <= z_flat.shape[-1]:
                return torch.mean((x - z_flat[:, :x.shape[-1]]) ** 2, dim=0)  # [D]
            else:
...
class Riyazi_Pareto_PCGrad_MGDA_Operator:
    ...
    def birlestir_ve_uygula_dagitik_gradyanlar(self,
        ...
        Pareto-PCGrad Dikgen Projeksiyon Birleştiricisi (Sheaf Glueing Mechanism).
        
        GPU'lar/Shard'lar anlamsal olarak tamamen farklı dosyaları işlediğinde gradyanlar
        birbirine zıt olabilir (g_i^T g_j < 0). Düz ortalama almak yıkıcı sönümlenmeye yol açar!
        Bu fonksiyon zıt gradyanları dikgenleştirir (g_i perp g_j) ve Pareto ağırlıklarıyla harmanlar.
        """
        ...
        if n == 1:
            ...
            for k, p in enumerate(trainable_params):
                if p.requires_grad:
                    p.grad = shard_gradyanlari[0][k].to(p.device)
            torch.nn.utils.clip_grad_norm_(trainable_params, max_norm=max_norm)
            optimizer.step()
            return torch.tensor([1.0], device=trainable_params[0].device)

        # 0. Birim Uzunluk Normalizasyonu (GradNorm): Devasa Faz Kayıp Ölçek Farklarını (84.0 vs 0.0003) Sıfırlama
        normalized_shards: List[List[torch.Tensor]] = []
        for g_list in shard_gradyanlari:
            norm_g = torch.sqrt(sum(torch.sum(g * g) for g in g_list)) + 1e-8
            normalized_shards.append([g / norm_g for g in g_list])

        # 1. PCGrad Çakışan Yönlerin Dikgenleştirilmesi (Birim-Uzunluk Gradyanlar Üzerinde)
        pc_grads: List[List[torch.Tensor]] = [[g.clone() for g in g_list] for g_list in normalized_shards]
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                dot_ij = sum(torch.sum(pc_grads[i][k] * normalized_shards[j][k]) for k in range(len(trainable_params)))
                if dot_ij < 0:
                    norm_sq_j = sum(torch.sum(normalized_shards[j][k] * normalized_shards[j][k]) for k in range(len(trainable_params))) + 1e-8
                    proj_coeff = dot_ij / norm_sq_j
                    for k in range(len(trainable_params)):
                        pc_grads[i][k] = pc_grads[i][k] - proj_coeff * normalized_shards[j][k]

        # 2. Dağıtık Gram Matrisi G hesabı
        G = torch.zeros((n, n), device=trainable_params[0].device, dtype=torch.float32)
        for i in range(n):
            for j in range(n):
                G[i, j] = sum(torch.sum(pc_grads[i][k] * pc_grads[j][k]) for k in range(len(trainable_params)))

        # 3. Pareto Optimizasyonu (QP Solver)
        alpha_star = self.coz_mgda_pareto_weights(G)

        # 4. Dikgenleştirilmiş Yıkımsız Birleşik Gradyan Hesabı ve Adım
        optimizer.zero_grad()
        for k, p in enumerate(trainable_params):
            if p.requires_grad:
                g_bileske_k = sum(alpha_star[i] * pc_grads[i][k] for i in range(n))
                p.grad = g_bileske_k.to(p.device)
...
class Hafiza_Izleyici_ve_VRAM_Denetci:
    ...
    """
    def __init__(self, cihaz: Union[torch.device, str], kritik_esik_yuzde: float = 0.85):
        ...
        if torch.cuda.is_available() and self.cihaz.type == "cuda":
            self.toplam_vram_mb = torch.cuda.get_device_properties(self.cihaz).total_memory / (1024 ** 2)
            self.esik_mb = self.toplam_vram_mb * kritik_esik_yuzde
        else:
            self.toplam_vram_mb = 0.0
            self.esik_mb = 0.0
    ...
    def yokla_ve_raporla(self, dugum_adi: str, adim_no: int = 0) -> Dict[str, float]:
        ...
        """
        if not (torch.cuda.is_available() and self.cihaz.type == "cuda"):
            return {"tahsis_mb": 0.0, "rezerve_mb": 0.0, "bos_mb": 0.0}

        tahsis_mb = torch.cuda.memory_allocated(self.cihaz) / (1024 ** 2)
        rezerve_mb = torch.cuda.memory_reserved(self.cihaz) / (1024 ** 2)
        bos_mb = self.toplam_vram_mb - tahsis_mb
        yuzde = (tahsis_mb / self.toplam_vram_mb * 100) if self.toplam_vram_mb > 0 else 0.0

        logger = logging.getLogger(__name__)
        logger.info(
            f"[VRAM Denetçi] Adım:{adim_no} | Düğüm:{dugum_adi} | "
            f"Aktif Tahsis: {tahsis_mb:.2f} MB / {self.toplam_vram_mb:.2f} MB "
            f"(%{yuzde:.1f}) | Rezerve: {rezerve_mb:.2f} MB"
        )

        if tahsis_mb > self.esik_mb:
            logger.warning(
                f"CRITICAL VRAM UYARISI! [{dugum_adi}] adımında VRAM %{self.kritik_esik_yuzde*100:.1f} eşiğini aştı "
                f"({tahsis_mb:.2f} MB > {self.esik_mb:.2f} MB). Acil VRAM Temizliği Başlatılıyor..."
            )
            torch.cuda.empty_cache()
...
try:
    ...
    try:
        from kulli_gpu.hardware_discovery import VeriyoluSorgulayicisi, DonanimArayici
    except ImportError:
