...
def _baglam() -> Baglam:
    ...
    for ad, tip in [("Y", U), ("Z", U)]:
        g = g.genislet(ad, tip)
    X, Y, Z = D("X"), D("Y"), D("Z")
    g = g.genislet("f", S.ok(X, Y))
    g = g.genislet("gg", S.ok(Y, Z))
    g = g.genislet("P", S.ok(Z, U))
    g = g.genislet("PY", S.ok(Y, U))
    g = g.genislet("QY", S.ok(Y, U))
    g = g.genislet("Q", S.ok(X, U))
    g = g.genislet("M", U)
    g = g.genislet("Fw", S.ok(D("M"), U))
    g = g.genislet("w", D("M"))
    g = g.genislet("fR", S.ok(G.R, G.R))
    return g
