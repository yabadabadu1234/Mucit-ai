...
def _geometri_baglami() -> Baglam:
    ...
    Dt = sonsuz_kucukler()
    X = D("X")
    g = g.genislet_t("sifir_kare", S.yol(R, _carp(SIFIR_R, SIFIR_R), SIFIR_R))
    g = g.genislet_t("eks_R", S.ok(R, R))
    g = g.genislet_t("X", U)
    g = g.genislet_t("x", X)
    Tx = teget_lifi(X, D("x"))
    g = g.genislet_t("teget_top", S.ok(Tx, S.ok(Tx, Tx)))
    g = g.genislet_t("teget_sifir", Tx)
    g = g.genislet_t("teget_eks", S.ok(Tx, Tx))
    g = g.genislet_t("teget_skaler", S.ok(R, S.ok(Tx, Tx)))
    return g
...
def dogrula_hepsi() -> List[Dict[str, str]]:
    ...
    gM = (g.genislet_t("V", U)
           .genislet_t("Vtop", S.ok(D("V"), S.ok(D("V"), D("V"))))
           .genislet_t("Vsf", D("V"))
           .genislet_t("Veks", S.ok(D("V"), D("V")))
           .genislet_t("Vsk", S.ok(R, S.ok(D("V"), D("V")))))
    dV = dual(M)
    dM = Modul(dV, D("dtop"), D("dsf"), D("deks"), D("dsk"))
    gMd = (gM.genislet_t("dtop", S.ok(dV, S.ok(dV, dV)))
             .genislet_t("dsf", dV)
             .genislet_t("deks", S.ok(dV, dV))
             .genislet_t("dsk", S.ok(R, S.ok(dV, dV))))
