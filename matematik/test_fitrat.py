...
def test_uc_temel_yapi():
    ...
    assert ayrisma.gecer_mi(zincir, ["A"], ["C"]) is False
    assert ayrisma.gecer_mi(zincir, ["A"], ["C"], ["B"]) is True
    assert ayrisma.gecer_mi(catal, ["A"], ["C"]) is False
    assert ayrisma.gecer_mi(catal, ["A"], ["C"], ["B"]) is True
    # Çarpışma tersine: şarta bağlamak AÇIYOR.
    assert ayrisma.gecer_mi(carpis, ["A"], ["C"]) is True
    assert ayrisma.gecer_mi(carpis, ["A"], ["C"], ["B"]) is False


def test_carpismanin_nesli_de_acar():
    g = ayrisma.Cizge(("A", "B", "C", "D"),
                      (("A", "B"), ("C", "B"), ("B", "D")))
    assert ayrisma.gecer_mi(g, ["A"], ["C"]) is True
    assert ayrisma.gecer_mi(g, ["A"], ["C"], ["D"]) is False
...
def test_iki_usul_rastgele_cizgelerde_uyusuyor():
    ...
    for _ in range(500):
        ...
        assert (ayrisma.gecer_mi(g, [x], [y], Z)
                is ayrisma.gecer_mi(g, [x], [y], Z, ne="d_yol")), (kenar, x, y, Z)
...
def test_bitisik_dugumler_hicbir_sartla_ayrilmaz():
    ...
    for _ in range(100):
        ...
        Z = r.sample(kalan, r.randint(0, len(kalan)))
        assert ayrisma.gecer_mi(g, [a], [b], Z) is False
...
def test_ortusen_kumeler_reddediliyor():
    ...
    with pytest.raises(ValueError):
        ayrisma.gecer_mi(g, ["A"], ["B"], ["A"])
    with pytest.raises(ValueError):
        ayrisma.gecer_mi(g, ["A"], ["A"])


def test_arka_kapi_karistiriciyi_buluyor():
    g = ayrisma.Cizge(("X", "Y", "Z"), (("Z", "X"), ("Z", "Y"), ("X", "Y")))
    assert ayrisma.gecer_mi(g, "X", "Y", ["Z"], ne="arka_kapı") is True
    assert ayrisma.gecer_mi(g, "X", "Y", [], ne="arka_kapı") is False
...
def test_arka_kapi_ardila_baglanmayi_reddediyor():
    ...
    assert ayrisma.gecer_mi(g, "X", "Y", ["M"], ne="arka_kapı") is False
    # Karıştırıcı yoksa boş küme doğru cevaptır:
    assert ayrisma.gecer_mi(g, "X", "Y", [], ne="arka_kapı") is True


def test_on_kapi_gozlenmemis_karistiricida_calisiyor():
    g = ayrisma.Cizge(("U", "X", "M", "Y"),
                      (("U", "X"), ("U", "Y"), ("X", "M"), ("M", "Y")))
    assert ayrisma.gecer_mi(g, "X", "Y", ["M"], ne="ön_kapı") is True
...
def test_on_kapi_yonlu_yol_kesilmiyorsa_reddediyor():
    ...
    g = ayrisma.Cizge(("X", "M", "Y"),
                      (("X", "M"), ("M", "Y"), ("X", "Y")))
    assert ayrisma.gecer_mi(g, "X", "Y", ["M"], ne="ön_kapı") is False


def test_b_ayrisma_tek_ortamda_tutani_kabul_etmiyor():
    o1 = ayrisma.Cizge(("X", "Y", "Z"), (("Z", "X"), ("X", "Y"), ("Z", "Y")))
    o2 = ayrisma.Cizge(("X", "Y", "Z"), (("Z", "X"), ("X", "Y")))
    assert ayrisma.gecer_mi(o2, ["Z"], ["Y"], ["X"]) is True
    assert ayrisma.gecer_mi(o1, ["Z"], ["Y"], ["X"]) is False
    assert ayrisma.gecer_mi(cizgeler=[o1, o2], X=["Z"], Y=["Y"],
                            Z=["X"], ne="b") is False
