...
\title{\textbf{Nefs-i Müdrike Mimarisi: 41 Melekenin Sağîr ve Kebîr Mertebelerinde Küllî Riyazî Formülasyonu}\\ \normalsize\textmd{(Tashihli nüsha --- 23 formül düzeltmesi; gerekçeler \texttt{docs/kaynak/TASHIH\_CETVELI.md} dosyasındadır)}}
...
H_t^{(1)} &= \text{Softmax}\left(\frac{(X_t W_q)(X_t W_k)^T}{\sqrt{d}}\right) X_t W_v \\
...
\begin{align}
\text{Murad}_{\text{aslî}} &= \text{Tefsir}(S_{\text{müphem}}, \text{Siyak}, \text{Sibak}) = \text{Softmax}\left( \frac{S_{\text{müphem}} W_{\text{tefsir}} [\text{Siyak} \oplus \text{Sibak}]^T}{\sqrt{d}} \right) [\text{Siyak} \oplus \text{Sibak}] W_v \\
