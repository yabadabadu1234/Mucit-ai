...
def test_ayni_sekil_oran_bir_olarak_bulunuyor():
    k = sk.sekil_kaidesi(_c([(3, 4), (5, 2), (7, 7)], [(3, 4), (5, 2), (7, 7)]))
    assert k.ad == "oran|oran"
    assert k.satir.deger == Fraction(1) and k.sutun.deger == Fraction(1)
    assert k.kestir(np.zeros((9, 11))) == (9, 11)


def test_olcekleme_orani_tam_kesirle():
    k = sk.sekil_kaidesi(_c([(2, 3), (4, 1)], [(6, 9), (12, 3)]))
    assert k.satir.deger == Fraction(3) and k.sutun.deger == Fraction(3)
    assert k.kestir(np.zeros((5, 5))) == (15, 15)


def test_kucultme_orani():
    k = sk.sekil_kaidesi(_c([(6, 6), (9, 3)], [(2, 2), (3, 1)]))
    assert k.satir.deger == Fraction(1, 3)
    assert k.kestir(np.zeros((12, 6))) == (4, 2)


def test_devrik_capraz_kiple_yakalaniyor():
    k = sk.sekil_kaidesi(_c([(3, 5), (2, 7)], [(5, 3), (7, 2)]))
    assert k.ad == "capraz|capraz"
    assert k.kestir(np.zeros((4, 9))) == (9, 4)


def test_sabit_sekil():
    k = sk.sekil_kaidesi(_c([(3, 5), (2, 7), (9, 1)], [(1, 1), (1, 1), (1, 1)]))
...
def test_oran_sabitten_once_tercih_ediliyor():
    ...
    k = sk.sekil_kaidesi(_c([(2, 2), (2, 2)], [(2, 2), (2, 2)]))
    assert k.ad == "oran|oran"
...
def test_kayan_nokta_yuvarlamasi_sahte_kaide_uretmiyor():
    ...
    k = sk.sekil_kaidesi(_c([(3, 3), (6, 6)], [(7, 7), (14, 14)]))
    assert k.satir.deger == Fraction(7, 3)
...
def test_egitim_kumesinde_kapsam_ve_isabet():
    d = sk.kume_olc(arc.yukle_hepsi("training"))
...
def test_degerlendirme_kumesinde_de_tutuyor():
    ...
    d = sk.kume_olc(arc.yukle_hepsi("evaluation"))
