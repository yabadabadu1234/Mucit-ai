...
# PyTorch FX Graph Tracer kütüphanesi kontrolü
_TORCH_FX_AVAILABLE = False
try:
    import torch
    import torch.fx as fx
    _TORCH_FX_AVAILABLE = True
    logger.debug("[PredictiveEngine] PyTorch FX Graph Compiler successfully linked!")
except ImportError:
    logger.debug("[PredictiveEngine] PyTorch FX not directly imported; running native C-API DAG Compiler.")
...
class ErkenDevletEngine:
    ...
    def on_hesapla(self, operasyon_grafik_veya_model: Any) -> List[ExecutionNode]:
        ...
        Gelen model veya grafik ne olursa olsun (PyTorch FX, CUDA Graph, C++ DAG):
        1. Grafiği sembolik olarak izler (Symbolic Trace).
        2. Tüm düğümleri, VRAM adreslerini ve gerçek bayt ayak izlerini dinamik çıkarır.
        3. Topolojik sıralı 'ExecutionNode' listesi döndürür.
        """
        with self._lock:
            self.dag_nodes.clear()
            self.current_step_index = 0
            
            # 1. YOL: PyTorch FX Graph Module İzleme (Symbolic Trace)
            if _TORCH_FX_AVAILABLE and hasattr(operasyon_grafik_veya_model, "eval"):
                try:
                    traced_graph = fx.symbolic_trace(operasyon_grafik_veya_model)
                    node_idx = 0
                    for node in traced_graph.graph.nodes:
                        if node.op in ("call_module", "call_function", "call_method"):
                            ...
                            # Düğüm parametrelerinden tensör VRAM ayak izini dinamik okuma
                            if hasattr(node, "meta") and "tensor_meta" in node.meta:
                                t_meta = node.meta["tensor_meta"]
                                shape = getattr(t_meta, "shape", [1024, 1024])
                                dtype = getattr(t_meta, "dtype", None)
                                elem_sz = 4
                                ...
                                for d in shape:
                                    num_el *= max(1, d)
                                calc_bytes = num_el * elem_sz

                            exec_node = ExecutionNode(
                                node_id=node_idx,
                                op_name=f"fx_{node.op}_{node.target}",
                                virtual_ptr=v_ptr,
                                byte_size=calc_bytes,
                                target_gpu_id=node_idx % max(1, self.allocator.physical_gpus)
                            )
                            self.dag_nodes.append(exec_node)
                            node_idx += 1

                    if self.dag_nodes:
                        logger.debug(f"[ErkenDevletEngine] PyTorch FX traced {len(self.dag_nodes)} pure DAG execution nodes.")
                        return self.dag_nodes
                except Exception as fx_exc:
                    logger.debug(f"[PyTorch FX Trace Notice] Fallback to C-API Graph Parser: {fx_exc}")

            # 2. YOL: Sözlük veya CUDA Graph C-API Düğüm Taraması
            if isinstance(operasyon_grafik_veya_model, dict):
                ...
                for idx, item in enumerate(raw_nodes):
                    if isinstance(item, dict):
                        ...
                        t_gpu = item.get("target_gpu_id", idx % max(1, self.allocator.physical_gpus))
                        s_bytes = item.get("scratchpad_bytes", 0)
                    else:
                        op_n = str(item)
                        v_ptr = None
                        b_sz = 0
                        t_gpu = idx % max(1, self.allocator.physical_gpus)
                        s_bytes = 0
