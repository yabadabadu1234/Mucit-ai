...
#pragma unroll
    ...
    half sa = sa2.x + sa2.y;
    sa2 = {sa, sa};
    ((F*)w)[i] = F(exp2f(NEXP_HALF_LOG2_E / (1.0f + exp2f(NLOG2_E * float(((F*)w)[i])))) - 1.0f + rotator1(elapsed_t[b] + h * CLONE_N + i + tt));
...
#pragma unroll
    ...
    half sa = sa2.x + sa2.y;
    sa2 = {sa, sa};
    ((half*)w[cur])[i] = w_delta(((half*)w[cur])[i], elapsed_t[b_id] + h * N + i + tt);
    __syncthreads();
